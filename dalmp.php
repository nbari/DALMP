 ...
 * # $db->memCache(MEMCACHE_HOSTS);
 *
 * @link http://code.dalmp.com
 * @copyright Nicolas de Bari <nbari@dalmp.com>
 * @author Nicolas de Bari <nbari@dalmp.com>
...
class DALMP {
	...
	public function PExecute() {
		...
		if ($this->_stmt->execute()) {
			...
			trigger_error('error ' . __METHOD__ . ' ' . $this->getConnection($cn)->error, E_USER_WARNING);
			return false;
		}
	}
	...
	public function memCache($hosts, $compress = false) {
		...
		if (!function_exists('memcache_pconnect')) {
			if ($this->debug) {
				$this->add2log('memCache', __METHOD__, 'error', 'Memcache module PECL extension not found!');
			}
			return false;
		}
	...
	public function setCache($key, $object, $timeout = null, $cn = null, $uid = null) {
		$cn = isset($cn) ? $cn : $this->cname;
		$timeout = isset($timeout) ? $timeout : $this->timeout;
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, 'key: ' . sha1('DALMP' . $key . $cn . $uid) , "key: $key, object: $object, timeout: $timeout, cn: $cn, uid: $uid");
		}
		return ($this->isMemcacheConnected()) ? $this->_memcache->set(sha1('DALMP' . $key . $cn . $uid) , $object, $this->memCacheCompress, $timeout) : false;
	}
	
	public function getCache($key, $cn = null, $uid = null) {
		$cn = isset($cn) ? $cn : $this->cname;
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, 'key: ' . sha1('DALMP' . $key . $cn . $uid) , "key: $key, cn: $cn, uid: $uid");
		}
		return ($this->isMemcacheConnected()) ? $this->_memcache->get(sha1('DALMP' . $key . $cn . $uid)) : false;
	}
	...
	public function CacheFlush($key = null) {
		if ($key) {
			if ($this->debug) {
				$this->add2log('memCache', __METHOD__, 'flush key:' . $key);
			}
			$cn = $this->cname;
			return ($this->isMemcacheConnected()) ? $this->_memcache->delete(sha1('DALMP' . $key . $cn)) : false;
		} else {
			if ($this->debug) {
				$this->add2log('memCache', __METHOD__, 'flush all');
			}
			return ($this->isMemcacheConnected()) ? $this->_memcache->flush() : false;
		}
	}
	
	public function CacheExecute($timeout, $sql = null, $cn = null, $uid = __METHOD__) {
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, "sql: $sql, timeout: $timeout");
		}
		...
		if ($cache = $this->getCache($sql, $cn, $uid)) {
			return $cache;
		} else {
			$cache = $this->GetAll($sql, $cn);
			if (!$this->setCache($sql, $cache, $timeout, $cn, $uid)) {
				if ($this->debug) {
					$this->add2log('memCache', __METHOD__, 'error', 'setCache, memcache daemon not running or responding');
				}
				trigger_error('error ' . __METHOD__ . 'setCache, memcache daemon not running or responding');
			}
			return $cache;
		}
	}
	...
	public function CacheGetAll() {
		$args = func_get_args();
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, implode(', ', $args));
		}
		return call_user_func_array(array(
			$this,
			'CacheExecute'
	...
	public function CacheGetRow($timeout, $sql = null, $cn = null, $uid = __METHOD__) {
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, "sql: $sql, timeout: $timeout, uid: $uid");
		}
		if (!is_numeric($timeout)) {
			$cn = $sql;
			$sql = $timeout;
		...
		if ($cache = $this->getCache($sql, $cn, $uid)) {
			...
			$cache = $this->GetRow($sql, $cn, $uid);
			if (!$this->setCache($sql, $cache, $timeout, $cn, $uid)) {
				if ($this->debug) {
					$this->add2log('memCache', __METHOD__, 'error', 'setCache, memcache daemon not running or responding');
				}
				trigger_error('error ' . __METHOD__ . " $sql setCache, memcache daemon not running or responding", E_USER_WARNING);
			}
			return $cache;
		}
	}
	
	public function CacheGetCol($timeout, $sql = null, $cn = null, $uid = __METHOD__) {
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, "sql: $sql, timeout: $timeout, uid: $uid");
		}
		if (!is_numeric($timeout)) {
	...
	public function CacheGetOne($timeout, $sql = null, $cn = null, $uid = __METHOD__) {
		if ($this->debug) {
			$this->add2log('memCache', __METHOD__, "sql: $sql, timeout: $timeout, uid: $uid");
		}
		if (!is_numeric($timeout)) {
	...
	public function CacheGetASSOC($timeout, $sql = null, $cn = null, $uid = __METHOD__) {
		if ($this->debug) {
			$this->add2log(__METHOD__, "sql: $sql, timeout: $timeout");
		}
		if (!is_numeric($timeout)) {
			$cn = $sql;
			$sql = $timeout;
		...
		if ($cache = $this->getCache($sql, $cn, $uid)) {
			...
			if (!$this->setCache($sql, $cache, $timeout, $cn, $uid)) {
				if ($this->debug) {
					$this->add2log(__METHOD__, 'error', 'setCache, memcache daemon not running or responding');
				}
				trigger_error('error ' . __METHOD__ . " $sql setCache, memcache daemon not running or responding", E_USER_WARNING);
			}
			return $cache;
	...
	protected function CacheP() {
		...
		if ($cache = $this->getCache(serialize($args) , end($args) , $fetch)) {
			return $cache;
		} else {
			call_user_func_array(array(
				$this,
			...
			if (!$this->setCache(serialize($args) , $cache, $timeout, end($args) , $fetch)) {
				if ($this->debug) {
					$this->add2log('memCache', __METHOD__, 'error', 'setCache, memcache daemon not running or responding');
				}
				trigger_error('error ' . __METHOD__ . "->$fetch setCache, memcache daemon not running or responding", E_USER_WARNING);
			}
			return $cache;
		}
	}
	...
	public function Sopen() {
		...
		if (!$rs) {
			...
			trigger_error('error ' . __METHOD__ . 'setCache, memcache daemon not running or responding', E_USER_WARNING);
		}
	...
	public function Sread($sid) {
		...
		if ($this->isMemcacheConnected()) {
			if ($cache = $this->getCache($sid, $this->dalmp_sessions_cname)) {
				if ($this->debug) {
					$this->add2log('memCache', __METHOD__, 'session cached', $cache);
				}
				$this->debug = $this->debug2 ? true : false;
				return $cache;
			} else {
				...
				if (!$this->setCache($sid, $cache, get_cfg_var('session.gc_maxlifetime') , $this->dalmp_sessions_cname)) {
					if ($this->debug) {
						$this->add2log('memCache', __METHOD__, 'error', 'setCache, memcache daemon not running or responding', $sid);
					}
					trigger_error('error ' . __METHOD__ . 'setCache, memcache daemon not running or responding', E_USER_WARNING);
				}
			}
			if ($this->debug) {
				$this->add2log('sessions', __METHOD__, 'memCache Error', $cache);
			}
			$this->debug = $this->debug2 ? true : false;
			return $cache;
		} else {
			...
			if ($this->debug) {
				$this->add2log('sessions', __METHOD__, 'NO memCache', $return);
			}
			$this->debug = $this->debug2 ? true : false;
			return $return;
		}
	...
	public function Swrite($sid, $data) {
		...
		if ($this->isMemcacheConnected()) {
			$rs = $this->setCache($sid, $data, get_cfg_var('session.gc_maxlifetime') , $this->dalmp_sessions_cname);
			if (!$rs) {
				$write2db = true;
				if ($this->debug) {
					$this->add2log('memCache', __METHOD__, 'error', 'setCache, memcache daemon not running or responding', $sid, $data);
				}
				trigger_error('error ' . __METHOD__ . 'setCache, memcache daemon not running or responding', E_USER_WARNING);
			} else {
				if ($this->debug) {
					$this->add2log('memCache', __METHOD__, 'setCache -> ', $sid, $data, $this->dalmp_sessions_cname);
				}
			}
		}
		if (!isset($rs) || isset($write2db) || isset($ref)) {
			...
			if ($this->debug) {
				$this->add2log('sessions', __METHOD__, 'setCache -> ', $sql, $data, $expiry, $sid, $ref, $this->dalmp_sessions_cname);
			}
		}
		$this->debug = $this->debug2 ? true : false;
		return true;
	...
	public function SessionStart($start = null, $cn = null) {
		...
		if (isset($_SESSION)) {
			...
			if ($this->debug_sessions) {
				$this->add2log('sessions', __METHOD__, "start: $start, cn: $this->dalmp_sessions_cname, sessions table: $this->dalmp_sessions_table");
			}
		}
	}
